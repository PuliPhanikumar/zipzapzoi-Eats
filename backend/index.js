...
// ════════════════════════════════════════════════════════
// ZOI AI AGENT (GEMINI INTEGRATION)
// ════════════════════════════════════════════════════════
    ...
    try {
        if (!process.env.GEMINI_API_KEY) {
            return res.status(503).json({
                response: "🔴 **Zoi AI is currently offline.**\n\nI need a `GEMINI_API_KEY` configured in the server's `.env` file to process your requests! Please alert the administrator."
            });
        }

        const { history } = req.body;
        // history is an array of messages generated by the client: [{ role: 'user', text: '...' }, { role: 'bot', text: '...' }]
        ...
        if (history && Array.isArray(history)) {
            ...
            conversationContents.push({ role: 'user', parts: [{ text: "Hello Zoi! What can you do?" }] });
        }

        const systemInstruction = `You are Zoi, the official AI Agent and Personal Concierge of ZipZapZoi Eats. 
You are an expert in food, calories, nutrition, recipes, and you handle ZipZapZoi Eats customer support perfectly.
You must adopt a warm, modern, and helpful persona, using emojis to keep the conversation vibrant.

ZipZapZoi Platform Logic:
- Refunds: Within 60 seconds of order via Tracking, or submit a support ticket. Refunds hit the Zoi Wallet or original payment in 24 hours.
- ZoiPass Subscription: Unlimited free deliveries, 10% off, exclusive deals.
- Gamification (ZipPoints): Earn 10pts/order, 50pts for feedback. Redeem for discounts.
- Platform: Customers, Partners (Restaurants), Riders, and POS users.
- Food/Calories: Feel free to give accurate nutritional data or healthy eating advice based on general knowledge.
- Never write broken HTML. Format your responses elegantly in standard Markdown (using **bold**, *italics*, and lists).

You are intelligent, so infer the context and provide a 100% full-fledged response taking advantage of your LLM capabilities.`;
        ...
        console.error("Zoi AI Error:", error);
        res.status(500).json({ 
            response: "🔴 **Oops, my neural network had a hiccup!**\n\nI couldn't process your request right now. Please try again in a moment." 
        });
    }
});
